...
async function fixHwpxLayout(buf, ratios) {
  ...
  // ---------- 칸 안을 줄마다 문단으로 나누고 내어쓰기를 준다 ----------
  //  한글 변환기는 칸 하나를 문단 하나로만 만든다. 그래서 줄바꿈이 글자
  //  사이에 들어가고, 줄이 길어 다음 줄로 넘어가면 넘어간 부분이 왼쪽 끝으로
  //  돌아온다. 받아서 한글에서 손볼 때마다 SHIFT+TAB 으로 맞춰야 하는 이유다.
  //  줄마다 문단으로 쪼개고 '왼쪽 여백 + 내어쓰기' 를 넣어 두면
  //  글을 고쳐 줄이 밀려도 머리기호 뒤에 계속 맞춰진다.
  const EM = 700;                       // 본문 7pt (1pt = 100 HWPUNIT)
  const HALF = Math.round(EM / 2);
  const isWide = (ch) => /[\u1100-\u11FF\u2E80-\uA4CF\uAC00-\uD7A3\uF900-\uFAFF\uFE30-\uFE4F\uFF00-\uFF60\u2460-\u24FF\u25A0-\u25FF\u2190-\u21FF]/.test(ch);
  const widthOf = (t) => [...t].reduce((w, ch) => w + (isWide(ch) ? EM : HALF), 0);
  //  "1." "2)" "■" "-" "–" 처럼 줄머리에 오는 기호
  const MARKER_RE = /^(?:\d+[.)]|[■●○▪▫◆◇□▶\-\u2013\u2014])[ \u00A0]+/;

  let paraCnt = (/<hh:paraProperties itemCnt="(\d+)"/.exec(head) || [, '1'])[1];
  paraCnt = Number(paraCnt);
  const paraCache = new Map();
  const paraIdFor = (left, hang) => {
    if (!para0 || (!left && !hang)) return null;
    const key = `${left}:${hang}`;
    if (paraCache.has(key)) return paraCache.get(key);
    const id = paraCnt++;
    const def = para0[0]
      .replace(/id="0"/, `id="${id}"`)
      .replace(/<hh:intent value="[-\d]*"\/>/, `<hh:intent value="${-hang}"/>`)
      .replace(/<hh:left value="[-\d]*"\/>/, `<hh:left value="${left}"/>`);
    head = head.replace('</hh:paraProperties>', `${def}</hh:paraProperties>`);
  ...
  xml = xml.replace(/<hp:tc ([^>]*)>([\s\S]*?)<\/hp:tc>/g, (whole, tcAttrs, body) => {
    const next = body.replace(/<hp:p\b([^>]*)>([\s\S]*?)<\/hp:p>/g, (pWhole, pAttrs, pBody) => {
      // 가운데 정렬해 둔 칸(제목 행·기관명·참여인력)은 건드리지 않는다
      const curId = (/paraPrIDRef="(\d+)"/.exec(pAttrs) || [])[1];
      if (centerParaId && Number(curId) === centerParaId) return pWhole;
      ...
      if (!text) return pWhole;

      const lines = text.split('\n');
      // 나눌 것도 맞출 것도 없으면 그대로 둔다
      const plain = lines.length === 1 && !MARKER_RE.test(lines[0]) && !/^[ \u00A0]/.test(lines[0]);
      if (plain) return pWhole;

      return lines.map((raw) => {
        const lead = (/^[ \u00A0]*/.exec(raw) || [''])[0];
        const rest = raw.slice(lead.length);
        const marker = (MARKER_RE.exec(rest) || [''])[0];
        const left = widthOf(lead) + widthOf(marker);
        const hang = widthOf(marker);
        const useId = paraIdFor(left, hang);
        const attrs = pAttrs
          .replace(/id="\d+"/, `id="${pid++}"`)
          .replace(/paraPrIDRef="\d+"/, `paraPrIDRef="${useId === null ? (curId || 0) : useId}"`);
        return `<hp:p${attrs}><hp:run charPrIDRef="${charRef}">` +
               `<hp:t>${rest}</hp:t></hp:run>${segs}</hp:p>`;
      }).join('');
    });
    return `<hp:tc ${tcAttrs}>${next}</hp:tc>`;
  });

  head = head.replace(/(<hh:paraProperties itemCnt=")\d+/, `$1${paraCnt}`);
