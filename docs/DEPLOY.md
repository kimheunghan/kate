...
| **활동 로그 (`wr.audit_logs`)** | **남습니다** |

활동 로그가 남는 이유 — `user_id` 는 `SET NULL` 이 되지만 `username`·`user_name`
을 값으로 복사해 두어 계정이 지워져도 누구였는지 알 수 있습니다.
...
`주간보고_전체(N개 주차).zip` 의 N 은 **실제로 담은 주차 수**입니다. 대상 목록이
아니라 담은 결과로 셉니다. 예전에는 목록 기준으로 이름을 짓고 담을 때 건너뛰어,
"2개 주차" 인데 하나만 든 묶음이 나갔습니다.
